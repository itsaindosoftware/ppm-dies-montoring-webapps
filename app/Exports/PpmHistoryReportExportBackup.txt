...
class PpmHistoryReportExport implements FromCollection, WithHeadings, WithStyles, WithTitle, ShouldAutoSize
{
    ...
    public function collection()
    {
        ...
        foreach ($groupedHistories as $histories) {
            $index++;
            $rows->push($this->buildRow($histories, $index));
        }

        return $rows;
    }

    protected function buildRow($histories, int $index): array
    {
        $history = $histories->first();

        $processSlots = $histories
            ->pluck('process_type')
            ->filter()
            ->unique()
            ->map(function ($process) {
                return ucwords(str_replace('_', ' + ', $process));
            })
            ->values()
            ->pad(7, '-')
            ->take(7)
